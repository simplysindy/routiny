...
const TaskDetailWrapper: FC<TaskDetailWrapperProps> = ({ task: initialTask }) => {
  ...
  return (
    <div>
      {task.task_type === 'single-day' ? (
        <SingleDayTaskDisplay task={task} onStepComplete={handleSingleDayStepComplete} />
      ) : (
        <MultiDayTaskDisplay task={task} onStepComplete={handleMultiDayStepComplete} />
      )}
    </div>
  );
};
