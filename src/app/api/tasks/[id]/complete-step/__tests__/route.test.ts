...
describe('POST /api/tasks/[id]/complete-step', () => {
  ...
  it('should mark single-day step as complete', async () => {
    ...
    const request = new NextRequest('http://localhost/api/tasks/task-123/complete-step', {
      method: 'POST',
  ...
  it('should mark multi-day step as complete', async () => {
    ...
    expect(response.status).toBe(200);
    expect((data.data.ai_breakdown as MultiDayBreakdown).day_1[0].completed).toBe(true);
    expect(data.data.status).toBe('in_progress');
  });
  ...
  it('should handle backward compatibility with string[] format', async () => {
    ...
    expect(response.status).toBe(200);
    expect(data.data.ai_breakdown[0]).toEqual({ text: 'Step 1', completed: true });
  });
