...
describe("TaskInput", () => {
  ...
  type TaskStoreMockState = {
    createTask: typeof mockCreateTask;
    tasks: Task[];
    loading: boolean;
    error: string | null;
  };
  ...
  type SelectorHookMock<TState> = {
    mockImplementation: <T>(
      impl: (selector: (state: TState) => T) => T
    ) => unknown;
  };

  const mockUseTaskStore =
    useTaskStore as unknown as SelectorHookMock<TaskStoreMockState>;
  const mockUseAuthStore =
    useAuthStore as unknown as SelectorHookMock<AuthStoreMockState>;
  ...
  beforeEach(() => {
    ...
    mockUseTaskStore.mockImplementation(
      <T,>(selector: (state: TaskStoreMockState) => T) => {
        const store: TaskStoreMockState = {
    ...
    mockUseAuthStore.mockImplementation(
      <T,>(selector: (state: AuthStoreMockState) => T) => {
        const store: AuthStoreMockState = {
          user: mockUser,
          session: null,
          isLoading: false,
          isInitialized: true,
  ...
  it("should validate input too short", async () => {
    ...
    await waitFor(() => {
      expect(
        screen.getByText(/task must be at least 3 characters/i)
      ).toBeInTheDocument();
    });
  ...
  it("should clear error when user starts typing", async () => {
    ...
    expect(
      screen.queryByText(/failed to create task/i)
